...
advertise = data.table::fread("https://statlearning.com/Advertising.csv",
                              verbose = T, stringsAsFactors = F, data.table = F, drop = 1) 

# There are some updates on the original CSV files. Here we rename the columns
...
#  Particle swarm optimization (PSO) with simple constraints
library(pso)
psoptim(par = c(0,0,0),  fn = function(x){
  if( x[1] + x[2] + x[3]  > 100) return(0)
  cat("TV = ", x[1], ", radio = ", x[2], ", newspaper = ", x[3], "\n")
  return(predict(ad_fit, newdata = data.frame(TV = x[1], radio = x[2], newspaper = x[3] ))) 
}, lower = c(0,0,0), upper = c(100,100,100), control = list(maxit = 100, s = 20, fnscale = -1))
...
# Add a new column as the default value in 0 or 1
Default$default_num =  as.numeric(Default$default) - 1

# An example of MLE for logistic regression
# Negative log-likelihood function
neglogLL = function(beta0, beta1) {
  # Convert factor variable "default" into 0 and 1
  y = Default$default_num 
  # exp (b0 + b1x1 + ....)
  eBX = exp(beta0 +  beta1 * Default$balance)
  # Return the sum of negative log-likelihood
  return(- sum(y * log( eBX/( 1 + eBX)) + (1 - y) * log( 1 - (eBX/( 1 + eBX)) ) ))
}
stats4::mle(neglogLL, start=list(beta0 = 0, beta1 = 0), method = "Nelder-Mead")

# Coefficients:
#   beta0           beta1 
#   -10.652058220   0.005499188 

# How about using the built-in glm() to fit the logit model?
glm(default ~ balance, data = Default, family = "binomial")

# Let's say if we just fit a linear model
lm_default = lm(default_num ~ balance, data = Default)

# The actual vs. predicted probabilities
qplot(x = 1:2500, y = predict(lm_default, newdata = data.frame(balance = 1:2500))) + theme(text = element_text(size=20)) +
  geom_point(aes(x = Default$balance, y = Default$default_num), colour = "red") +  ylim(-0.15,1) + 
  ylab("Probability of Default") + xlab("Balance") 

# How about a logistic regression model? 
logit_default = glm(default_num ~ balance, data = Default, family = "binomial")
qplot(x = 1:2500, y = predict(logit_default, newdata = data.frame(balance = 1:2500), type = "response") ) +
  theme(text = element_text(size=20)) +
  geom_point(aes(x = Default$balance, y = Default$default_num), colour = "red") +  ylim(-0.15,1) + 
  ylab("Probability of Default") + xlab("Balance")


stats4::mle(neglogLL, start=list(beta0 = 0, beta1 = 0), method = "BFGS")
...
exp(glm_default$coefficients[3])
...
heart = read.table("https://statlearning.com/Heart.csv", header = T, sep = ",")[,-1]
# Check out https://archive.ics.uci.edu/ml/datasets/heart+Disease for more info.
