...
test_mae[[1]] = mae(scaled_test$Balance, y_hat_test1)
...
test_mae[[2]] = mae(scaled_test$Balance, y_hat_test2) # 0.2654454
...
test_mae[[3]] = mae(scaled_test$Balance, y_hat_test3) # 0.1882051
...
test_mae[[4]] = mae(scaled_test$Balance, y_hat_test4) # 0.124302
...
train_test_mae <- sapply(list("train_mae" = scaled_train, "test_mae" = scaled_test), 
                         function(k) {
                           Map(function(f) {
                             tt_reponse_var <- all.vars(f)[1];
                             tt_mae <- mae(k[, tt_reponse_var], predict(lm(f, data = scaled_train), k))
...
validation <- Map(function(f) LOOCV_lm_mae(f, scaled_train),
                  c(Balance ~ Limit + Income,
                    ...
                    Balance ~ . ^ 2,
                    Balance ~ (Limit + Income) ^ 2,
                    Balance ~ (Limit + Income + Student) ^ 2)) %>% unlist
#plot
tibble(
